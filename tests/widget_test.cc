...
tiny_gui::LayoutItem _root{tiny_gui::LayoutBuilder().dimensions_relative(1,1).spacing(5), tiny_gui::VerticalLayout};
...
void drawWave (tiny_gui::FrameBufferView& fbv, int width, int height, int tick){
    tiny_gui::Painter::fill_color(fbv, {0});
    for (int i = 0; i < width; i++){
        int py = static_cast<int>(height * std::sin(i + tick));
        tiny_gui::Painter::draw_pixel(fbv, i, py, tiny_gui::WHITE);
    }
}
...
int main () {
    ...
    tiny_gui::FrameBuffer fb{frame_buffer.data(), WIDTH, HEIGHT};
    ...
    tiny_gui::FrameBufferView fbv = fb.view(_pane1._computedDims);
    ...
    while (ctx.isAlive()){
        ...
        drawWave(fbv, _pane1._computedDims.width(), _pane1._computedDims.height(), tick);
        tick += 1;
